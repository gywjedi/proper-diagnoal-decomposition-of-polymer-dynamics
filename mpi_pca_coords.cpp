...
// Writes ALL CSV outputs into ./_phi_/ :
//   phix<M>.csv, phiy<M>.csv, phiz<M>.csv
//   lamx<M>.csv, lamy<M>.csv, lamz<M>.csv
...
// ------------------------- Config -------------------------
static const std::string PATH_DUMPS = "./_coord_/"; // folder with dump.*.txt
static const std::string PATH_PHI   = "./_phi_/";   // OUTPUT folder for phi*, lam*
...
std::vector<std::string> discover_files_mpi(int rank) {
    ...
    if (rank == 0) {
        if (!fs::exists(PATH_DUMPS))
            throw std::runtime_error("Directory not found: " + PATH_DUMPS);
        for (const auto& e : fs::directory_iterator(PATH_DUMPS)) {
...
void pca_one_coord_for_molecule(const std::vector<std::string>& fnames,
    ...
    // Ensure output directory exists (create once, race-safe)
    static bool made_dir = false;
    if (!made_dir) {
        // Let rank 0 create; others wait
        int rank; MPI_Comm_rank(MPI_COMM_WORLD, &rank);
        if (rank == 0) {
            std::error_code ec;
            if (!fs::exists(PATH_PHI)) fs::create_directories(PATH_PHI, ec);
            if (ec) std::cerr << "Warning: failed to create " << PATH_PHI << ": " << ec.message() << "\n";
        }
        MPI_Barrier(MPI_COMM_WORLD);
        made_dir = true;
    }

    // Save CSVs into _phi_
    const char c = (coord == XU ? 'x' : coord == YU ? 'y' : 'z');
    std::ostringstream phiname; phiname << PATH_PHI << "phi" << c << mol << ".csv";
    std::ostringstream lamname; lamname << PATH_PHI << "lam" << c << mol << ".csv";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const int ns = static_cast<int>(fnames.size());
        if (ns == 0) {
            if (rank == 0) std::cerr << "No dump.*.txt files found in " << PATH_DUMPS << "\n";
            MPI_Finalize();
            return 0;
        }
        ...
        for (int mol = start_m; mol < end_m; ++mol) {
            pca_one_coord_for_molecule(fnames, mol, nx, XU, w_time, w_space); // -> _phi_/phix*.csv, lamx*.csv
            pca_one_coord_for_molecule(fnames, mol, nx, YU, w_time, w_space); // -> _phi_/phiy*.csv, lamy*.csv
            pca_one_coord_for_molecule(fnames, mol, nx, ZU, w_time, w_space); // -> _phi_/phiz*.csv, lamz*.csv
        }
